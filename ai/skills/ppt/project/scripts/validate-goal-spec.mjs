...
// Lesson-plan preview mode keeps repeated copy non-blocking so teachers can
// inspect the generated deck before copy refinement is enabled.
const ALLOW_REPEATED_COPY = /^(1|true|yes)$/i.test(
  String(process.env.DASHI_ALLOW_REPEATED_COPY || ''),
);
...
export function validateGoalSpec(spec, options = {}) {
  ...
  if (!ALLOW_REPEATED_COPY) validateDeckRepeatedCoreCopy(deckCoreCopyUsages, errors);
