    ...
    <div class="relative" ref="containerRef">
      <nav class="flex relative" :style="{ transform: 'translate3d(0,0,0.01px)' }">
        ...
        >
          ...
          >
            <a
              @click.prevent="e => !item.disabled && handleClick(e, index, item)"
              @keydown.prevent="e => !item.disabled && handleKeyDown(e, index, item)"
...
const activeIndex = ref<number>(
  props.modelValue 
    ? Math.max(0, props.items.findIndex(i => i.value === props.modelValue))
...
const handleClick = (e: Event, index: number, item: GooeyNavItem) => {
...
const handleKeyDown = (e: KeyboardEvent, index: number, item: GooeyNavItem) => {
...
watch(() => props.modelValue, (newVal) => {
  ...
  const index = props.items.findIndex(i => i.value === newVal);
