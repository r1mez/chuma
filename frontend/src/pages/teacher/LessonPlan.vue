  ...
  <div class="lesson-plan-page">
    <section class="page-heading">
      <div>
        ...
        <h2>教案生成</h2>
        <p>基于课程知识图谱、教材资料和班级真实学情生成可编辑的 PPTX 课件。</p>
      </div>
...
import { createLessonPlan, downloadLessonPlan, getCourseSections, getLessonPlan, getLessonPlans, type CourseSection, type LessonPlanItem, type LessonPlanStatus } from '@/api/lessonPlans'
...
function layoutText(layout: string) {
  return ({ title: '标题页', objectives: '学习目标', review: '学情回顾', knowledge_map: '图谱定位', concept: '核心概念', example: '例题演示', difficulty_focus: '易错点', activity: '课堂活动', summary: '小结' } as Record<string, string>)[layout] || '内容页'
}
...
async function generateLessonPlan() {
  ...
  try {
    ...
    ElMessage.success('教案任务已提交，完成后可查看提纲并下载 PPTX')
    syncPolling()
  } catch (error: any) {
